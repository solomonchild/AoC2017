...
auto to_int_vec(const Strings& vec) {
	std::vector<int> ret;
	std::for_each(vec.begin(), vec.end(), [&ret] (const std::string& el){
		ret.push_back(std::stoi(el));
	});
	return ret;
}
...
unsigned checksum_1(const std::vector<Ints>& ints) {
	std::vector<int> diffs;
	for(const auto& row : ints) {
		auto max_el = *std::max_element(row.begin(), row.end());
		auto min_el = *std::min_element(row.begin(), row.end());
		diffs.push_back(max_el - min_el);
	}
	return std::accumulate(diffs.begin(), diffs.end(), 0);
}

unsigned checksum_2(const std::vector<Ints>& ints) {
	std::vector<int> diffs;
	for(auto row : ints) {
...
"1763	464	182	1932	1209	640	545	931	1979	197	1774	174	2074	1800	939	161",
	...
	std::cout << "Checksum: " << checksum_2(nums);
}
